...
% This file is needed to run 'program_p.m'. Basically, it initializes
% matrices and defines parameters based on wat, elem, crop, soil, and ofe
% files loaded in from 'program_p.m'.
...
% col1: ofe id
% col2: day of the month (I convert to a cumulative day over the entire
...
% units) 
QS=fixedinputdata(4);
% Reference DP plant/soil export coefficient (mg/L = ppm)
mewTS=fixedinputdata(5);
% Reference temperature for soil (C)                   
TR=fixedinputdata(7);
% Reaction constant for bound P to DP (m^3/kg)
...
% Application location and amount in kg/ha
applnofe=scheddata(:,4:7);
% Convert amount to kg/m^2
applnofe=applnofe./10000;
...
% Particulate P accumulated in the buffer (kg/m^2)
bufferP=zeros(numsimdays,numofe);
% Mixing depth (m)
...
% Particulate P accumulated in the buffer (kg/m^2)
bufferPkgm2=zeros(numsimdays,numofe);
...
for id=numofe+1:1:watrow %start at second day
    ...
    if ofe>1
        lostPfertUP=lostPfert(i-1,ofe-1);
        lostPplntsoilUP=lostPplntsoil(i-1,ofe-1);
        lostPlatUP=lostPlatf(i-1,ofe-1);   
    else
        lostPfertUP=0;
        lostPplntsoilUP=0;
        lostPlatUP=0;
   ...
   % Define depth of mixing layer
    ...
    % Run P function
    out=dailyofePsimTransBuff(id,i,k,tstep,tau,hmix(i-1,ofe),mixdepth,ofeArea(ofe),kdissolv,stpconc(ofe),mewTS,QS,TR,plowday,applnday,applnamt,avalsolPfert(i-1,ofe),lostPfertUP,lostPplntsoilUP,lostPlatUP,avgT(id),ovldf(id),latf(id),perc(id),sed(id),sedfrac,ofe,buffernum);
    ...
    bufferPkgm2(out(3),numofe)=out(11)/applnarea;
...
% Daily p losses (kg)
dailyPdatakg=[avalsolPfert,lostPfert,lostPplntsoil,lostPeros,lostPlatf,lostPperc,totalPlostOvlfnosed,bufferP];
% Daily p losses (kg/m^2)
dailyPdatakgm2=[avalsolPfertkgm2,lostPfertkgm2,lostPplntsoilkgm2,lostPeroskgm2,lostPlatfkgm2,lostPperckgm2,totalPlostOvlfnosedkgm2,bufferPkgm2];
