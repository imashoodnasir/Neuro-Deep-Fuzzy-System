...
% This program calculates P transport (dissolved and particulate
...
for h=1:1:length(userinputfiles)
    ...
    if userinputdata(37,1)==1 % Buffer.
        ...
        % hillslope base in kg/ha)
         ...
         txtstr2=sprintf('pRun%02.0f',h);
         txtfile=[outputfolder,'\',txtstr1,'_',txtstr2,'.txt'];        
         dlmwrite(txtfile,monthlyPhillavgdata);
        ...
        txtstr2=sprintf('pRun%02.0f',h);
        txtfile=[outputfolder,'\',txtstr1,'_',txtstr2,'.txt'];        
        dlmwrite(txtfile,yearlyPhillavgdata);
        
    else
        % Running the sub program P model for all ofe's
        run('sub_program_p');
...
% colm 1-3: p left in the top layer for ofe 1 to 3 (kg/m^2)
% colm 4-6: p left in the bottom layer for ofe 1 to 3 (kg/m^2)
% colm 7-9: total p lost in overland flow for ofe 1 to 3 (kg/m^2)
% colm 10-12: dissolved p lost in overland flow for ofe 1 to 3 (kg/m^2)
% colm 13-15: adsorbed p lost in overland flow for ofe 1 to 3 (kg/m^2)
% colm 16-18: p lost in lateral flow for ofe 1 to 3 (kg/m^2)
% colm 19-21: p lost in shallow percolation for ofe 1 to 3 (from top layer
% into bottom layer) (kg/m^2)
% colm 22-24: p lost in deep percolation for ofe 1 to 3 (kg/m^2)
% colm 25: (buffer runs only) particulate p trapped by buffer
% (kg/m^2)

% monthly text files (average for all simulations at hillslope base)
% colm1: month
% colm2: adsorbed p lost in overland flow (kg/ha)
% colm2: dissolved p lost in overland flow (kg/ha)
% colm3: p lost in lateral flow (kg/ha)
% colm4: p lost in deep percolation (kg/ha)

% yearly text files (average for all simulations at hillslope base)
% row1: adsorbed p lost in overland flow (kg/ha)
% row2: dissolved p lost in overland flow (kg/ha)
% row3: p lost in lateral flow (kg/ha)
% row4: p lost in deep percolation (kg/ha)
