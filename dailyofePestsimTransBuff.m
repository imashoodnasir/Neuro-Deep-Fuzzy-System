...
function pesticideDailyDATA=dailyofePestsimTransBuff(id,day,yr,tstep,apc,applnamt,mixdepth,hmixprev,ps,pw,thetaS,rootdepth,applnday,plowday,thalf,pestleftTop,pestleftBot,lostPestovldfWUP,lostPestovldfSUP,lostPestlaftUP,rain,evap,qovlf,qlatf,qperc,sedfrac,ofe,buffernum)
...
if qovlf==0 % No runoff
    if ofe<buffernum
        ...
        pestlostovlfW=lostPestovldfWUP;
        pestlostovlfTot=lostPestovldfWUP;        
        pestlostovlfS=0;        
        bufferPest=lostPestovldfSUP;
    end;
...
elseif qovlf>0
    %Pesticide lost with sediment in overland flow
    %Calculate sediment concentration in overland flow
    if qovlf>0
        ...
        if ofe<buffernum
            ...
            if pestlostovlfTot<=pestleftTopnow
                ...
                %Sorbed pesticide lost in overland flow (kg/m^2)
                pestlostovlfS=0;
                bufferPest=(pestlostovlfTot*apc*pw*sedfrac)/(1+sedfrac*(apc*pw-1));
            else
                pestlostovlfTot=pestleftTopnow;
                pestleftTopnow=0;
                %Dissolved pesticide lost in overland flow (kg/m^2)
...
pesticideDailyDATA=[id,ofe,day,yr,pestlostovlfTot,pestlostovlfW,pestlostovlfS,pestlostlatf,pestlostshalperc,pestlostperc,pestleftTopnow,pestleftBotnow,bufferPest];
