...
% This file is needed to run 'program_pesticide.m'. It initializes matrices
% and defines parameters based on wat, elem, crop, soil, and parameter
% files loaded in from 'program_pesticide.m'.
...
a=[elemdata(:,1:2),elemdata(:,24)];         % Sediment (kg/m)
sednewlst=zeros(croprow,1);                 % New matrix (sediment in col 1, sm in col 2)
for i=1:1:elemrow
    % Sediment
    if a(i,3)>0
        idlook=a(i,1:2);                    % Identify sediment event day
        x=find(cropdata(:,2)==idlook(2),1); % Identify associated row id for the day (one row for each ofe so have to adjust for this by subtracting 1)
        sednewlst(x+idlook(1)-1,1)=a(i,3);
    end;     
end;
...
for s=1:1:sr
    ...
    % Plowing (included planting b/c of disruption to soil)
    if plowdate(s)>0
        if scheddata(s,1)==1 % first year
...
% Sediment (kg)
sedlst=zeros(numsimdays,numofe);
...
%% Check to see if pesticide initiation impacts output

% load in pesticide left in top and bottom from last day of previous 10
% year run and set this equal to the first day of this run

% initpestTop=importdata('pestTop58.txt');
% initpestBot=importdata('pestBot58.txt');
% 
% pestleftTop(1,:)=initpestTop;
% pestleftBot(1,:)=initpestBot;
...
for id=numofe+1:1:watrow %start at second day
    i=daylst(id);
    ofe=ofelst(id); 
    % Upslope Pesticide Contributions (dissolved P in runnoff and lateral flow)
    if ofe>1
...
dailyPestdatakg=[pestleftTop,pestleftBot,pestlostovlfTot,pestlostovlfW,pestlostovlfS,pestlostlatf,pestlostshalperc,pestlostperc];
% Daily pesticide losses (kg/m^2)
dailyPestdatakgm2=[pestleftTopkgm2,pestleftBotkgm2,pestlostovlfTotkgm2,pestlostovlfWkgm2,pestlostovlfSkgm2,pestlostlatfkgm2,pestlostshalperckgm2,pestlostperckgm2];
...
% convert to kg/ha
for k=1:1:mpy
    monthlyavgPesterosbymonthhill(k)=mean(monthlyPesterosbymonthhill(:,k))*10000*(ofeLength(numofe)/sum(ofeLength)); % must multiply by length last ofe/sum of all ofe lengths to get in right units for end of the hillslope b/c wepp .wat inputs are cumulative)
...
for k=1:1:numyrs
    yearlyPesteroshill(k)=yearlylostPesteros(k,numofe).*(ofeLength(numofe)/sum(ofeLength)); % must multiply by length last ofe/sum of all ofe lengths to get in right units for end of the hillslope b/c wepp .wat inputs are cumulative)
